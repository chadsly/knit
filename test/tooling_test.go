...
func TestChromiumExtensionSidePanelHostsComposer(t *testing.T) {
	...
	required := []string{
		...
		`id="headerHelper" class="helper hidden"`,
		...
		`id="videoGuidance" class="video-guidance hidden"`,
		`.video-guidance {`,
		`.video-guidance.active {`,
		`id="activityState" class="preview-loading activity-state hidden"`,
		`id="queueIndicator" class="queue-indicator hidden"`,
		`.queue-indicator {`,
		`.queue-table {`,
		`.queue-table td {`,
		`.queue-link {`,
...
func TestChromiumExtensionRecorderIncludesCaptureFlows(t *testing.T) {
	...
	required := []string{
		...
		`function openMainUIForAttempt(attemptID)`,
		...
		`data-preview-action="toggle-video-omit"`,
		...
		`Large clip needs a decision`,
		`omit_video_event_ids: previewDeliveryOptions.omitVideoEventIDs || []`,
		`Submission blocked until you choose how to handle the large clip in preview.`,
		`screenshot_data_url`,
		`video_data_url`,
		`URL.createObjectURL`,
