...
# Reads a Knit CLI payload JSON from stdin and invokes local OpenCode CLI.
# Emits a compact JSON result that Knit's CLI adapter can parse.

work_dir="${KNIT_CODEX_WORKDIR:-$(pwd)}"
run_id="opencode-cli-$(date +%s)"
payload_file="$(mktemp)"
prompt_file="$(mktemp)"
...
if [[ -n "${KNIT_CLI_LOG_FILE:-}" ]]; then
  ...
  log_file="$(mktemp "$output_dir/knit-opencode-${run_id}-XXXX.log")"
fi
...
if [[ -n "${KNIT_CLI_LOG_FILE:-}" ]]; then
  if run_opencode 1>&2; then
    printf '{"run_id":"%s","status":"accepted","ref":"%s"}\n' "$run_id" "$log_file"
  else
    status=$?
    echo "opencode cli failed (exit $status). See log: $log_file" >&2
    printf '{"run_id":"%s","status":"failed","ref":"%s"}\n' "$run_id" "$log_file"
    exit "$status"
  fi
else
  if run_opencode > "$log_file" 2>&1; then
    printf '{"run_id":"%s","status":"accepted","ref":"%s"}\n' "$run_id" "$log_file"
  else
    status=$?
    echo "opencode cli failed (exit $status). See log: $log_file" >&2
